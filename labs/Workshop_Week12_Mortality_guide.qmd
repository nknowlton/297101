...
Plot the **scatter**, **fitted line**, and **prediction band** using `geom_ribbon`. 
...
What happens when we split the data at other points? Explore this on your own.
...
4. **Confirm with a statistical test** when appropriate
:::
...
**Diagnostics: Base R**
The diagnostic plots can be generated quickly in base R with the plot command on a "lm" object. We will create ggplot versions to better understand how these plots are made.
...
```{r resid-vs-fitted, fig.height=4, fig.width=7}
#| message: false
...
**Try it** Plot the square root absolute valued standardized residuals vs the fitted values.
...
***Answer*** there is an upward trend, suggesting heteroscedasticity, i.e. variance increases with poverty rate. This means our model is less precise for high-poverty counties. Did we see evidence of that last week when we predictd a county with 75% poverty?
...
**Answer** We see some deviation from normality starting after the 1st quantile and slowly increasing as the quantiles increase. Overall I would say that this is somewhat normal and likely close enough to use for regression modeling. Next year you will learn how to add other model terms into a regression and those new "sinks" of variability can improve the overall fit of the model generally and in diagnostics.
:::
...
**Answer**: Overall it looks like there are no single influential points.
...
### Statistical Diagnostics: Formal Hypothesis Tests
...
## Part 3. Transformation: Linear-Log Model
...
We'll try a Linear-Log transformation on both mortality rate and poverty rate.
...
# Fit Linear-Log model
lm.mortality.log <- lm(Mortality_Rate_num ~ log(pov_norm), data = cancer_log)

# Compare summaries side by side
model_comp <- tibble(
  Model = c("Linear (Y ~ X)", "Linear-Log (Y ~ log X)"),
...
coefs_linear     <- coef(lm.mortality)
coefs_linearlogx <- coef(lm.mortality.log)

tibble(
  Model = c("Linear", "Linear–Log"),
  Intercept = c(coefs_linear[1], coefs_linearlogx[1]),
  Slope = c(coefs_linear[2], coefs_linearlogx[2]),
  Interpretation = c(
    sprintf("1 unit increase in pov_norm is associated with %.4f increase in mortality per 100k", coefs_linear[2]),
    sprintf("1%% increase in pov_norm is associated with approximately %.4f increase in mortality per 100k", coefs_linearlogx[2] * 0.01)
  )
)
...
aug_logx <- augment(lm.mortality.log)
...
p1 <- aug_logx |>
  ...
  geom_smooth(formula = y ~ x, se = TRUE) +
  geom_hline(yintercept = 0, linetype = 2) +
  labs(title = "Linear–Log model: Residuals vs Fitted",
       subtitle = "Both axes on original mortality scale; look for randomness about zero",
       x = "Fitted mortality per 100k",
       y = "Residual (deaths per 100k)")
...
p2 <- aug_logx |>
  ...
  geom_smooth(formula = y ~ x, se = TRUE) +
  geom_hline(yintercept = 0, linetype = 2) +
  labs(title = "Linear–Log model: Scale–Location",
       subtitle = "Target a horizontal band for homoscedasticity",
       x = "Fitted mortality per 100k",
       y = "Sqrt(|standardized residual|)")
...
p3 <- aug_logx |>
  ...
  geom_qq(alpha = 0.5) +
  geom_qq_line() +
  labs(title = "Linear–Log model: Q-Q plot of standardized residuals",
       subtitle = "Closer to the line indicates more normal residuals",
       x = "Theoretical quantiles",
       y = "Sample quantiles")
...
p4 <- aug_logx |>
  ...
  labs(title = "Linear–Log model: Cook's distance",
       subtitle = "Smaller values and fewer spikes indicate fewer influential cases",
       x = "Index",
       y = "Cook's D")
...
| Diagnostic | Linear Model | Linear-Log Model | Winner? |
|------------|--------------|---------------|---------|
| **Linearity** | Random around 0 | Funnel shape | Linear |
| **Equal Variance** | Upward- Funnel | Upward- No funnel | Linear-Log |
| **Normality** | OK | OK | Draw |
| **Influential Points** | None | None | Draw |

**Your Conclusion:** Based on the diagnostics, which model better satisfies the LINE assumptions?

**Answer** The diagnostics for each model are very similar. I would choose the linear-log model as explainng percent changes in poverty vs normalized values makes more intuitive sense, additionally using a log of poverty gave another 2% increase in R2.
...
comparison <- tibble(
  ...
  `Linear-Log p-value` = c(bp_log$p.value, NA, sh_log$p.value),
  `Linear Statistic` = c(bp$statistic, dw$statistic, sh$statistic),
  `Linear-Log Statistic` = c(bp_log$statistic, dw_log$statistic, sh_log$statistic)
) |>
...
::: callout-note
**Predictions and Coverage Comparision**

Now we will plot both models to inspect there prediction windows visually.

```{r coverage-comparison}
pred_pi <- predict(lm.mortality.log, interval = "prediction")

aug_pred_logx <- augment(lm.mortality.log) |>
  bind_cols(as_tibble(pred_pi, .name_repair = "unique")) |>
  # After bind_cols you will have columns like fit...9, lwr...10, upr...11.
  # Rename them to stable names regardless of their numeric suffixes.
  rename(
    fit_pi = tidyselect::starts_with("fit"),
    lwr_pi = tidyselect::starts_with("lwr"),
    upr_pi = tidyselect::starts_with("upr")
  ) |>
  # Create a clean predictor column for plotting
  mutate(log_pov = `log(pov_norm)`)

# 3) Plot Y vs log X with prediction band for the linear–log fit
p_ll <- aug_pred_logx |>
  ggplot(aes(x = log_pov, y = Mortality_Rate_num)) +
  geom_point(alpha = 0.4) +
  geom_ribbon(aes(ymin = lwr_pi, ymax = upr_pi), alpha = 0.2) +
  geom_line(aes(y = fit_pi), linewidth = 1) +
  labs(
    title = "Linear–Log model on original mortality scale",
    subtitle = "X-axis is log poverty; Y-axis is mortality per 100k",
    x = "log(pov_norm)",
...
```{r coverage_two_models}
# Coverage for linear model
aug_lin <- augment(lm.mortality) |>
  cbind(as_tibble(predict(lm.mortality, interval = "prediction"))) |>
  as_tibble()

coverage_linear <- aug_lin |>
  mutate(in_band = between(Mortality_Rate_num, lwr, upr)) |>
  summarise(
    `Points in Band` = sum(in_band, na.rm = TRUE),
    `Total Points` = n(),
    `Coverage %` = 100 * `Points in Band` / `Total Points`
  )

# Coverage for linear–log model
coverage_logx <- aug_pred_logx |>
  mutate(in_band = between(Mortality_Rate_num, lwr_pi, upr_pi)) |>
  summarise(
    `Points in Band` = sum(in_band, na.rm = TRUE),
    `Total Points` = n(),
    `Coverage %` = 100 * `Points in Band` / `Total Points`
...
bind_rows(
  coverage_linear |> mutate(Model = "Linear (Y ~ X)"),
  coverage_logx   |> mutate(Model = "Linear–Log (Y ~ log X)")
) |>
  relocate(Model)
```
...
## Weeks 10 to 12: Linear Regression
...
::: callout-tip
**Workflow for Regression Models**
