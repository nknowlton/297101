...
```
We see that medium income for black, native american, asian and hispanic are <= 80% complete. That might lead to some issues when we create models with the data as we need complete cases (no missing values) to estimate coefficients. So, lets remove those additional med income values. 
...
Yes, the `all_poverty` slope is within -3.5, which is plausible.
...
The `Multiple R-squared:` value is about 0.013, meaning 1.3% of the variability in `Mortality_Rate` is explained by the `all_poverty` variable. This is very low and although significant statistically, it likely to not be a helpful model.  
...
```
Whoa! A mortality rate over 100%. 

> **⚠️ Reality Check**: Our model predicts over 100,000 deaths per 100,000 people for 75% poverty rate. This seems unrealistic because:
> 1. You can only die once (so 100% is the theoretical maximum)
> 2. Very few counties actually have 75% poverty rates
> 3. We're **extrapolating** beyond our data range, which is dangerous!
> 
...
The predicted deaths are still 38/100k. To tell if this value is reasonable, we need to place it in context. Let's use a quick histogram of all mortality rates and see where this falls.
