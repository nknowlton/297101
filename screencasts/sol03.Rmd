...
Because the data are rounded, it is essentially discretised.
The histogram is also discretising the magnitudes according to the number of
bins or the width of the bins.
When these don't align we get oddness.

Let's try aligning them by specifying the binwidth to be a similar resolution:

```{r}
ggplot(data=quakes) +
  geom_histogram(mapping=aes(x=mag), binwidth = 0.1)
ggplot(data=quakes) +
  geom_histogram(mapping=aes(x=mag), binwidth = 0.2)
ggplot(data=quakes) +
  geom_histogram(mapping=aes(x=mag), binwidth = 0.3)
ggplot(data=quakes) +
  geom_histogram(mapping=aes(x=mag), binwidth = 0.4)
```
...
We can see that most earthquakes are between 4 and 5 magnitude (measure of location or center) with the most popular magnitude (the mode) around 4.5. The range is from 4.0 to 6.4 or so (measure of spread), and the shape is such that most earthquakes tend to be smaller, and we have just a few large ones (measure of shape).
...
Our conclusion regarding the shape of the distribution is the same. Measure of center would be magnitude 4.5 (value of highest density), or earthquakes are typically between say 4 and 5 magnitude. Range is from 4.0 to 6.4. The shape is asymmetric with most earthquakes being smaller with fewer large ones.
...
```{r}
ggplot(data=quakes) +
  geom_density(mapping=aes(x=mag), adjust=1)
ggplot(data=quakes) +
  geom_density(mapping=aes(x=mag), adjust=0.5) # gives less smoothing
ggplot(data=quakes) +
  geom_density(mapping=aes(x=mag), adjust=2) # gives more smoothing
```
...
Yes, the conclusoin is the same: Center (median) is at about 4.6 magnitude, most quakes between say 4.2 and 4.9 magnitude (in the box) and measure of spread is similar from 4.0 to 6.4. Shape can also be inferred from the position of the box compared to the range.
...
```{r}
ggplot(data=quakes) +
  geom_boxplot(mapping=aes(x=mag), col='blue', fill='yellow')
ggplot(data=quakes) +
  geom_histogram(mapping=aes(x=mag), binwidth=0.2, col='blue', fill='yellow')
ggplot(data=quakes) +
  geom_density(mapping=aes(x=mag), col='blue', fill='yellow')
ggplot(data=quakes) +
  geom_density(mapping=aes(x=mag), col='transparent', fill='yellow') # no colour
```
...
It's important when using boxplots that the inference from them (i.e. what your reader takes from the plot) is that the distribution has one "peak" or is unimodal. If this isn't the case, the boxplot may misinform and another plot would be better (histogram, density etc).

In this case we see there's two modes (or possibly more) - i.e. earthquakes are either shallow or deep, and there aren't many in the middle (which is where the box is on the boxplot!)
...
The shape and spreads of shallow or deep earthquake magnitudes are similar - most earthquakes are smaller and a few larger, the ranges are pretty similar (4 to 6 or more). However the typical values or measure of location differs: Shallow quakes tend to be a bit larger than deeper quakes.

9. Try switching to `fill=how_deep` or `col=how_deep` above instead of `y=how_deep` (or in addition if you like).

```{r}
ggplot(data=quakes_cat) +
  geom_boxplot(mapping=aes(x=mag, y=how_deep, fill=how_deep)) +
  guides(fill = 'none')
```
...
```{r}
ggplot(data=quakes_cat) +
  geom_density(mapping=aes(x=mag, fill=how_deep), alpha=0.5)
```

```{r}
ggplot(data=quakes_cat) +
  geom_histogram(mapping=aes(x=mag, fill=how_deep), binwidth=0.2)
```

Note that with the histogram the bars stack, so you only truly see the shape of
the group that starts at y=0. The other groups start at the top of that group!

We can use the position argument to change this:

```{r}
ggplot(data=quakes_cat) +
  geom_histogram(mapping=aes(x=mag, fill=how_deep), 
                 binwidth=0.2,
                 position='dodge') # side by side histograms. Not preferred!
ggplot(data=quakes_cat) +
  geom_histogram(mapping=aes(x=mag, fill=how_deep), 
                 binwidth=0.2,
                 position='identity',
                 alpha=0.5) # one on top of the other, add transparency.
```
