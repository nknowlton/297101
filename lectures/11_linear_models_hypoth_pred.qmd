...
format: 
  revealjs:
    ...
    css: ["knowlton.css", "regression-lab.css"]
    theme: sky
    slide-number: true
    incremental: true
    smaller: true
...
Choose whether the target is the mean response or one new outcome. Predict
which interval is appropriate before revealing both intervals.
:::
...
plotdata <- donkey |> arrange(Heartgirth)
plotdata$fit_raw <- predict(mod, newdata=plotdata)
plotdata$fit_power <- exp(predict(mod2, newdata=plotdata))

g1 <- ggplot(donkey, aes(x=Heartgirth, y=Bodywt)) +
  geom_point() +
  geom_line(data=plotdata, aes(y=fit_raw), colour="dodgerblue") +
  labs(title="lm(Bodywt ~ Heartgirth)")
g2 <- ggplot(donkey, aes(x=Heartgirth, y=Bodywt)) +
  geom_point() +
  geom_line(data=plotdata, aes(y=fit_power), colour="dodgerblue") +
  labs(title="lm(log(Bodywt) ~ log(Heartgirth))")
